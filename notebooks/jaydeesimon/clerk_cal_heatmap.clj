...
(ns jaydeesimon.clerk-cal-heatmap
...
;; function `square` defined in `src/jaydeesimon/custom.cljs`:

(def squared-viewer
  {:transform-fn clerk/mark-presented
   :render-fn
   '(fn [x]
      [:pre x "² is equal to " (jaydeesimon.custom/square x) "."])})
...
;; This is made possible by the code in `src/jaydeesimon/sci_viewers.cljs`. If
